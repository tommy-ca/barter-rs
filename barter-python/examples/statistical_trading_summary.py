        ...
        "executions": [
            {
                ...
                "initial_state": {
                    ...
                    "balances": [
                        {
                            "asset": "usdt",
                            "balance": {"total": 10000.0, "free": 10000.0},
                            "time_exchange": "2024-01-01T00:00:00Z",
                        },
                        {
                            "asset": "btc",
                            "balance": {"total": 0.0, "free": 0.0},
                            "time_exchange": "2024-01-01T00:00:00Z",
                        },
                    ],
                    "instruments": [{"instrument": "BTCUSDT", "orders": []}],
                },
            }
        ],
    }
...
def main():
    ...
    try:
        ...
        # Print summary
        print("Trading Summary (Risk-free return: 5%, Interval: Annual-365)")
        ...
        if instruments:
            ...
            for name, tear_sheet in instruments.items():
                ...
                print(
                    f"    Sharpe Ratio: {tear_sheet.sharpe_ratio.value} ({tear_sheet.sharpe_ratio.interval})"
                )
                print(
                    f"    Sortino Ratio: {tear_sheet.sortino_ratio.value} ({tear_sheet.sortino_ratio.interval})"
                )
                print(
                    f"    Calmar Ratio: {tear_sheet.calmar_ratio.value} ({tear_sheet.calmar_ratio.interval})"
                )
        ...
        if assets:
            ...
            for key, tear_sheet in assets.items():
                ...
                if tear_sheet.balance_end is not None:
                    ...
                    print(
                        f"    End Balance - Total: {balance.total}, Free: {balance.free}"
                    )
                print()
